...
def generate_starting_points_and_directions():
    for i in range(0, 20):
        yield [i, 0, 0, 1]
        yield [0, i, 1, 0]
        yield [i, 1, 1, 1]
...
def go_diag_right():
    ...
    for [start_i, start_j, dir_i, dir_j] in generate_starting_points_and_directions():
        prod = 1
        nr_in_prod = 0
        i = start_i
        j = start_j
        while i >= 0 and j >= 0 and i < 20 and j < 20:
            if data[i][j] == 0:
                nr_in_prod = 0
                prod = 1
            else:
                ...
                if nr_in_prod == 4:
                    ...
                    prod = prod / data[i-(3*dir_i)][j-(3*dir_j)]
                    nr_in_prod -= 1
